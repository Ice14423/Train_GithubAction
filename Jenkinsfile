 ...
 stage('Infrastructure (IaC)') {
            steps {
                dir('terraform') {
                    ...
                    // [แก้จุดที่ 2] ใช้ -var แต่เขียนแบบ Single Quote (')
                    // การใช้ '...' จะบอก Jenkins ว่า "ห้ามยุ่งกับข้อความข้างใน"
                    // แล้วให้ $GRAFANA_URL ถูกแปลงค่าโดย Linux Shell แทน (ปลอดภัยและชัวร์)
