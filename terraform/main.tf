...
# Provider สำหรับ us-east-1 (จำเป็นถ้าจะขยายไปใช้ WAF หรือ ACM ในอนาคต)
provider "aws" {
  alias  = "us_east_1"
  region = "us-east-1"
}
...
resource "aws_dynamodb_table" "grades_db" {
  ...
  # ใช้ Default Encryption (AWS Owned Key) ฟรีและเพียงพอ
...
# trivy:ignore:AVD-AWS-0132 (Ignore KMS check to save cost)
...
# [FIX] ใช้ OAC เพื่อความปลอดภัย (CloudFront เข้าถึง S3 ได้โดยไม่ต้องเปิด Public)
...
# [FIX] Policy อนุญาตให้ CloudFront OAC อ่านข้อมูลได้
...
# trivy:ignore:AVD-AWS-0011 (Ignore WAF check to save $5/mo)
...
# trivy:ignore:AVD-AWS-0066 (Ignore X-Ray tracing enabled check if not needed)
...
resource "aws_apigatewayv2_stage" "lambda_stage" {
  ...
  # Access Log settings removed to simplify/save cost (Optional)
  # ถ้าต้องการ Log ให้ uncomment ส่วนนี้และ resource CloudWatch Log Group
}
